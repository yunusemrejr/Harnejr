...
| Web UI coverage | The installed UI exposes provider generation, workers, review, provider probe, MCP check, Doctor, memory save, goal/topic/yolo controls, and user prompt editing. |
...
| SQLite session state | Current durable state is JSONL, not SQLite. |
